...
st.caption("Active Trading Engine • Hyperliquid & Bybit Rules")
...
if df_daily is not None and not df_daily.empty:
    ...
    if asset_meta["tier"] == 1:
        stop_dist_pct = 0.025 # 2.5% structural stop
        score = 8 if regime_1d == "BULLISH" else 6
    elif asset_meta["tier"] == 2:
        stop_dist_pct = 0.0497 # 4.97% structural stop (e.g. SOL $95.50 vs $100.50 entry)
        score = 8 if regime_1d == "BULLISH" else 6
    else:
        stop_dist_pct = 0.12 # Memes / Spot
        score = 5
