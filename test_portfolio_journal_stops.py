...
class TestJournal(unittest.TestCase):
    def test_add_and_retrieve(self):
        j = TradeJournal()
        idx = j.add(JournalEntry(asset="BTC-USD", direction="Long", entry=50000))
        self.assertEqual(j.all()[idx].asset, "BTC-USD")

    def test_close_trade_sets_pl_status(self):
        j = TradeJournal()
        idx = j.add(JournalEntry(asset="ETH-USD"))
        j.close_trade(idx, realized_pnl=150.0, exit_reason="Target hit")
        entry = j.all()[idx]
        self.assertEqual(entry.status, "Closed")
        self.assertEqual(entry.pl_status, "Win")
        self.assertEqual(entry.realized_pnl, 150.0)

    def test_close_trade_loss_status(self):
        j = TradeJournal()
        idx = j.add(JournalEntry(asset="ETH-USD"))
        j.close_trade(idx, realized_pnl=-50.0, exit_reason="Stopped out")
        self.assertEqual(j.all()[idx].pl_status, "Loss")

    def test_csv_export_contains_headers_and_rows(self):
        j = TradeJournal()
        j.add(JournalEntry(asset="SOL-USD", entry=150))
        csv_bytes = j.to_csv_bytes()
    ...
    def test_update_unknown_field_raises(self):
        j = TradeJournal()
        idx = j.add(JournalEntry())
        with self.assertRaises(AttributeError):
            j.update(idx, not_a_real_field=123)
